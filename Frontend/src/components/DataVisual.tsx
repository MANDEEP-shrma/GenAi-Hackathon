...
export default function DataVisual() {
  ...
  const computePieChartPaths = (data: { value: number }[]) => {
    ...
    data.forEach((item, index) => {
      ...
      // Generate the path if coordinates are valid
      if (!isNaN(startX) && !isNaN(startY) && !isNaN(endX) && !isNaN(endY)) {
        paths.push(
          <path
            key={index}
            d={`M16,16 L${startX},${startY} A16,16 0 ${largeArc},1 ${endX},${endY} Z`}
            fill={["#E57373", "#64B5F6", "#81C784", "#FFD54F"][index % 4]}
          />
        );
      }
      ...
      <div className="bg-white/5 backdrop-blur-sm p-6 rounded-lg border border-purple-700/20">
        ...
        <div className="relative h-40 flex items-center justify-center">
          <svg className="w-32 h-32" viewBox="0 0 32 32">
            {computePieChartPaths(demographicsData)}
          </svg>
        </div>
        ...
        <div className="h-48 relative">
          ...
          >
            ...
            ${growthData
              .map(
                (value, index) =>
                  `L ${index * (100 / (growthData.length - 1))} ${
                    50 - value * 0.5
                  }`
              )
              .join(" ")}`}
